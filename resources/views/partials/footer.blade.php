    ...
    <div class="container">
        <div class="row g-4">
            ...
            <div class="col-6 col-md-3">
                ...
                <p class="mb-1"><i class="bi bi-truck text-primary me-1"></i> Budapest: {{ number_format($shopShippingFee ?? 2500, 0, ',', '.') }} Ft / rendelés</p>
                <p class="mb-0 text-muted">Vidéki kiszállítás jelenleg nincs.</p>
            </div>
            <div class="col-md-3">
                <h6 class="fw-bold">Kapcsolat</h6>
                <p class="mb-1"><i class="bi bi-geo-alt text-primary me-1"></i> {{ \App\Models\SiteSetting::getValue('pickup_address') ?? 'Budapest XXIII. kerület, Soroksár' }}</p>
                <p class="mb-1"><i class="bi bi-telephone text-primary me-1"></i> <a href="tel:{{ preg_replace('/\s+/', '', $shopPhoneDisplay ?? '') }}">{{ $shopPhoneDisplay }}</a></p>
                <p class="mb-0 text-muted"><i class="bi bi-clock text-primary me-1"></i> {{ \App\Models\SiteSetting::getValue('opening_hours') ?? 'előzetes telefonos egyeztetés alapján' }}</p>
            </div>
        </div>
